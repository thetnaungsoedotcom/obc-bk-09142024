...
if (false === class_exists('WF_Licensing_wpforcessl')) {
    class WF_Licensing_wpforcessl
    {
        public $prefix = '';
        ...
        function __construct($params)
        {
            ...
            add_action('init', array($this, 'monitor_remote_actions'));
        ...
        function remote_action_refresh($action_params)
        {
            ...
            $data = apply_filters('wf_licensing_' . $this->prefix . '_query_server_data', $data);
        ...
        function init()
        {
            if (is_admin()) {
                ...
                wp_enqueue_script('wf_licensing_' . $this->prefix, $this->js_folder . 'wf-licensing.js', array(), 1.0, true);
                wp_localize_script('wf_licensing_' . $this->prefix, 'wf_licensing_' . $this->prefix, $vars);
            }
        } // init
        ...
        {
            $last_check = get_transient('wf_licensing_' . $this->prefix . '_last_check');
        ...
        function validate($license_key = '')
        {
            ...
            if (empty($license_key)) {
                if(strlen($license['license_key']) > 0){
                    $license_key = $license['license_key'];
                } else {
                    return false;
                }
            }
            set_transient('wf_licensing_' . $this->prefix . '_last_check', true, HOUR_IN_SECONDS * 8);
            ...
            if (is_wp_error($result)) {
                ...
                $out['license_key'] = $result['data']['license_key'];
        ...
        function query_licensing_server($action, $data = array())
        {
            ...
            $request_data = apply_filters('wf_licensing_' . $this->prefix . '_query_server_data', $request_data);
    ...
    } // WF_Licensing_wpforcessl
} // if WF_Licensing_wpforcessl
